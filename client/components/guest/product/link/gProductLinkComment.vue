<template>
    ...
    <v-card tile flat>
        ...
        <v-card-text v-else-if="$fetchState.error">
            <v-alert type="error" class="BoxShadow mb-0"> {{$fetchState.error.message}} </v-alert>
        </v-card-text>
            ...
            <v-card-text class="py-6 pb-0" v-if="Comments.length > 0">
                <v-sheet v-for="(comment, index) in CommentsMap" :key="index" class="d-flex mb-4">
                    ...
                    <v-sheet class="ml-2">
                        ...
                        <!--Reply-->
                        <GProductLinkReplyOfComment :product="product" :comment="comment"></GProductLinkReplyOfComment>
                    </v-sheet>
                </v-sheet>
            </v-card-text>
